...
const NewsPage = () => {
  ...
  useEffect(() => {
    let url = '/api/article';
    if (selectedCategory) url = '/api/article/category';
  ...
  return (
    <div className="company-page-container">
      ...
      {posts.map((post, idx) => {
        ...
        if (!likeStates[id]) {
          return <div key={idx} className="news-post">로딩...</div>;
        }
        const like = likeStates[id];
        return (
          <div className="company-post" key={idx}>
            <div
              className="company-post-main"
              onClick={() => {
